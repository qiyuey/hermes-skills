...
> **🌙 0点黄金窗口（实测发现）**：系统在 0:00 更新预约数据，type=5 普通会议室此时也可被抢占，且竞争者极少。脚本已针对 0:00-0:05 时段专门放行（即使系统返回"未开放"也继续尝试）。如果 cron 全天跑着，0 点时段命中率最高。
> 时间粒度：开始和结束时间必须是15分钟的整数倍（:00/:15/:30/:45），最短15分钟，时长须为15分钟的倍数。
