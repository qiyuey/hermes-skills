...
- **Install (preferred):** native installer, `curl -fsSL https://claude.ai/install.sh | bash`
- **Install (legacy/deprecated):** `npm install -g @anthropic-ai/claude-code`; migrate with `claude install` when possible
- **Auth:** run `claude` once to log in (browser OAuth for Pro/Max, or set `ANTHROPIC_API_KEY`)
- **Console auth:** `claude auth login --console` for API key billing
- **SSO auth:** `claude auth login --sso` for Enterprise
- **Check status:** `claude auth status` (JSON) or `claude auth status --text` (human-readable)
- **Health check:** `claude doctor` — checks auto-updater and installation health; if it hangs, don't stop there—run a direct print-mode smoke test because doctor can time out even when Claude Code itself works.
- **Version check:** `claude --version` (requires v2.x+)
- **Update:** `claude update` or `claude upgrade`
- **Install/update troubleshooting:** see `references/native-install-auto-updates.md` for identifying native vs npm/Homebrew installs and enabling native auto-updates.

## Hermes ↔ Claude Code 通信协议

Hermes 与 Claude Code 之间使用 **ACP（Agent Client Protocol）**——一个基于 JSON-RPC over stdio 的开放标准，输出为 NDJSON 事件流。详见 `references/acp-protocol.md`。

两种调用方式：
- **`terminal` + `claude -p`**：直接 shell subprocess，一次性任务，无 session 记忆
- **`terminal(pty:true)` + `claude-as-acp`**：ACP 协议，多轮有状态续接，结构化输出（推荐复杂任务）
...
### Bedrock adaptive thinking compatibility

When Claude Code is configured for Amazon Bedrock and newer Claude models (especially Opus 4.7 via Application Inference Profile ARN), a failing smoke test may show `thinking.type.enabled is not supported; use thinking.type.adaptive and output_config.effort`. Prefer fixing root cause by upgrading Claude Code (`claude update`) and re-testing Opus/default/Sonnet, rather than disabling thinking permanently. See `references/bedrock-adaptive-thinking.md` for the troubleshooting and verification recipe.

### Bedrock application-inference-profile vs system-defined profile

Corp accounts usually authorize **application-inference-profile** ARNs, not the AWS-managed **system-defined** `us.anthropic.claude-*` IDs. If `ANTHROPIC_DEFAULT_*_MODEL` points at the system profile while IAM only authorizes the application profile, every invoke returns `AccessDeniedException ... inference-profile/us.anthropic.claude-*` — superficially looking like missing perms but actually wrong-ARN-class.

**Fix:** Configure `~/.claude/settings.json` with an `env` block (Bedrock vars + ARNs) plus `modelOverrides` so `/model` routes correctly. This also keeps Bedrock env from leaking into sibling tools. Full diagnostic flow, settings.json template, IAM policy, and verification recipe: see `references/bedrock-application-inference-profile.md`.

### Bedrock 1M context window (`[1m]` alias)

Opus 4.7/4.6 and Sonnet 4.6/4.5 support a 1M-token context window via the `[1m]` suffix (e.g. `/model opus[1m]`). This is triggered by Claude Code's model alias resolution, not the Bedrock ARN — your `modelOverrides` stay the same regardless of whether you use `opus` or `opus[1m]`. Haiku and older models do not support 1M. Cost is ~5× for tokens beyond 200K. See `references/bedrock-application-inference-profile.md` → "1M context window" for the full support matrix, enable/disable methods, and Bedrock profile verification.
...
1. **Bedrock routing may reject default thinking config** — if a smoke test like `claude -p 'Reply exactly: CLAUDE_CODE_OK' --max-turns 1 --output-format json` returns `API Error: 400 "thinking.type.enabled" is not supported for this model`, the install/auth path is usually fine but the selected Bedrock model/router doesn't accept Claude Code's enabled-thinking payload. Retry with thinking disabled (`MAX_THINKING_TOKENS=0 CLAUDE_CODE_EFFORT_LEVEL=low claude -p ...`) or choose a compatible model/effort explicitly (`claude -p ... --model haiku --effort low`). For a durable fix, update `~/.claude/settings.json` env/model defaults rather than treating Claude Code as broken.
2. **Bedrock `AccessDeniedException` on invoke is usually wrong-ARN-class, not missing perms** — when corp accounts authorize **application-inference-profile** ARNs but env points to the **system-defined** `us.anthropic.claude-*` ID, every call fails with `is not authorized to perform: bedrock:InvokeModelWithResponseStream on resource: ...inference-profile/us.anthropic.claude-*`. Don't rotate credentials or escalate IAM until you've run `aws bedrock-runtime invoke-model --model-id "<application-profile-ARN>" --region <ARN-region> ...` directly. Success there proves the ARN works and the env vars are at fault. See `references/bedrock-application-inference-profile.md` for the full fix.
3. **User prefers concise, direct communication — no redundant closing phrases (e.g. "NO_REPLY", "Let me know if you need anything else").** When delivering results, state the facts and stop. Do not append polite filler or offer follow-up help unless explicitly asked.
...
5. **`--max-budget-usd` minimum is ~$0.05** — system prompt cache creation alone costs this much. Setting lower will error immediately.
...
9. **`--json-schema` needs enough `--max-turns`** — Claude must read files before producing structured output, which takes multiple turns.
10. **Trust dialog only appears once per directory** — first-time only, then cached.
11. **Background tmux sessions persist** — always clean up with `tmux kill-session -t <name>` when done.
12. **Slash commands (like `/commit`) only work in interactive mode** — in `-p` mode, describe the task in natural language instead.
13. **`--bare` skips OAuth** — requires `ANTHROPIC_API_KEY` env var or an `apiKeyHelper` in settings.
